...
int avx2_version(const int* source, int* dest) { 
  __builtin_assume_aligned(source, ALIGN);
    ...
    __m256i offset, base=_mm256_setzero_si256();
    __m256i mask1 = _mm256_set_epi32(7, 7, 7, 7, 7, 7, 7, 7);
    __m256i mask2 = _mm256_set_epi32(3, 3, 3, 3, 0, 0, 0, 0);
    // Mask to set all bits to 1
    __m256i mask = _mm256_set1_epi32(-1); 
    const int stride = AVX_WIDTH_BITS / (sizeof(int) * CHAR_BIT);
    for(int i = 0; i < N; i += stride){
      __m256i vec = _mm256_load_si256((__m256i*)&source[i]);
      //Shifting in by one int adding on each lane
      vec = _mm256_add_epi32 (vec, _mm256_slli_si256 (vec, 4));
      //Shifting in by two int adding on each lane
      vec = _mm256_add_epi32 (vec, _mm256_slli_si256 (vec, 8));
      offset = _mm256_permutevar8x32_epi32(vec, mask2);
      vec = _mm256_add_epi32 (vec, offset);
      // printf("vec::::::::::::");
      // print_vec(vec);
      vec = _mm256_add_epi32 (vec, base);
      // printf("base::::::::::::");
      // print_vec(base);
      _mm256_maskstore_epi32(&dest[i], mask, vec);
      //Setting base two right most value of right lane of vec
      base = _mm256_permutevar8x32_epi32(vec, mask1);
      // print_vec(base);
    }
